...
class ThreeAlgorithmAnalyzer:
    ...
    def plot_path_error_comparison(self):
        ...
        # 绘制三个算法的路径偏差
        ax.plot(time_data, los_error_smooth, 'blue', label='LOS', linewidth=1.5, alpha=0.8)
        ax.plot(time_data, gtlos_error_smooth, 'red', label='GTLOS', linewidth=1.5, alpha=0.8)
        ax.plot(time_data, gtlos_qiopid_error_smooth, 'green', label='GTLOS-QIOPID', linewidth=1.5, alpha=0.8)

        ax.set_xlabel('时间 (s)', fontsize=20)
        ax.set_ylabel('路径偏差 (m)', fontsize=20)
